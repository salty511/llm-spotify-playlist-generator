...
export const useSpotifyData = () => {
  ...
  const fetchTopTracks = async (token: string, timeRange: string): Promise<ProcessedTrack[]> => {
    try {
  ...
  const fetchTopArtists = async (token: string, timeRange: string): Promise<ProcessedArtist[]> => {
    try {
      const response = await fetch(
      ...
      const processedArtists: ProcessedArtist[] = data.items.map((artistObject: SpotifyArtist) => ({
  ...
  const fetchDataForTimeRange = async (token: string, timeRange: string): Promise<void> => {
    if (!token) return
